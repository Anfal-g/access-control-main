...
const verifyResident = async (qrData, res) => {
  try {
    ...
    // // 4. Check if already leaved
    // const lastLog = await EntryLog.findOne({ resident: residentMongo._id }).sort({ timestamp: -1 });

    // if (lastLog && lastLog.type === 'leave') {
    //   return res.status(400).json({ 
    //     success: false, 
    //     error: 'Resident already marked as leaved' 
    //   });
    // }

    const now = new Date();
    const timestamp = Math.floor(now.getTime() / 1000);
    const nextAction = 'leave';
...
try {
  ...
  await EntryLog.updateOne(
    ...
    res.status(200).json({
      success: true,
      message: 'Resident leaved successfully',
      action: nextAction,
      residentMongo
    });
  ...
  try {
   ...
   const now = new Date();
    ...
    return res.status(200).json({
      success: true,
      message: 'Visitor leaved successfully',
      action: nextAction,
      role: 'visitor',
      visitor,
    });
...
const verifyRequest = async (qrData, res) => {
  try {
    ...
    // Step 2: Check if an 'leave' log already exists
    ...
    //   return res.status(400).json({
    //     success: false,
    //     error: 'Visitor has already leaved'
    //   });
    // }
